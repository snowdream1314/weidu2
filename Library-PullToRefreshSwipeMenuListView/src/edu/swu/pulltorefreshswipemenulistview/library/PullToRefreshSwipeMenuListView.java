...
public class PullToRefreshSwipeMenuListView extends ListView implements OnScrollListener {
	...
	public boolean onTouchEvent(MotionEvent ev) {
		...
		case MotionEvent.ACTION_DOWN:
			...
			if (mTouchPosition == oldPos && mTouchView != null && mTouchView.isOpen() && mEnableSwipe) {//增加是否允许滑动判断
		...
		case MotionEvent.ACTION_MOVE:
			...
			if (mTouchState == TOUCH_STATE_X && mEnableSwipe) {//增加是否允许滑动判断
				if (mTouchView != null) {
					mTouchView.onSwipe(ev);
				}
				...
				} else if (dx > MAX_X && mEnableSwipe) {//增加是否允许滑动判断
					mTouchState = TOUCH_STATE_X;
